			...
			res.on('end', (chunk, encoding) => {
				...
				try {
					...
					_.each($('url'), (url) => {
						...
						const thumbnail = url.children.find(child => child.name === 'thumbnail');
						if (!!loc.children.length > 0) {
							...
							urls.push({
								...
								posttype: postType && postType.children && postType.children[0].data || null,
								thumbnail: thumbnail && thumbnail.children && thumbnail.children[0] && thumbnail.children[0].data || null,
								lang: sitemap.lang,
								action: action
							});
						} else {
